...
- [ ] std::vector
	- [ ] прочитать в книжке про std::vector + под какие операции оптимизирован std::vector и когда его лучше использовать вместо std::list.
	- [ ] как правильно удалить часть пакета из конца std::vector, оставив оставшуюся часть на потом.
	- [ ] std::memmove() / std::memcpy() для std::vector - когда что лучше использовать ? .
	- [ ] алгоритм ротатор (цикл. буфер).
