...
16 january.
- [ ] ? добавить возможность приближать изображение.
- [ ] переписать сеть на select неблокирующие сокеты.
- [x] emit fftCalculated.
- [ ] 1024 отсчётов где до этого было 512 = 4096 / n.
- [ ] исправить с потоками (сдеалть один STL поток (с синхронизацией на одной переменной) ), т.к. qthread не нужен в целом.
	- [x] убрать QThread::ProcessEvents().
	- [ ] переделать всё на STL потоки. 
